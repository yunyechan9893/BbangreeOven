...
public class NoticePagingResponseDto {
    ...
    public static NoticePagingResponseDto of(Page<NoticeResponseDto> noticePaging){
        ...
        if(!noticePaging.hasNext()){
            return NoticePagingResponseDto.newLastScroll(noticePaging.getContent(), noticePaging.getTotalPages()-1);
        }
        return NoticePagingResponseDto.newPagingHasNext(noticePaging.getContent(),
                                                    noticePaging.getTotalPages()-1, //page는 0부터 시작
                                                    noticePaging.getPageable().getPageNumber()+1);
    }
    //마지막 페이지인 경우
    private static NoticePagingResponseDto newLastScroll(List<NoticeResponseDto> noticePaging, long lastPage) {
        return newPagingHasNext(noticePaging, lastPage, LAST_PAGE);
    }

    //마지막 페이지가 아닌 경우
    private static NoticePagingResponseDto newPagingHasNext(List<NoticeResponseDto> noticePaging, long lastPage, long nextPage) {
